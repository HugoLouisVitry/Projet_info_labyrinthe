...
#    print("begin loop")
    while heap or heap_reverse:
        ...
        if heap and heap_reverse:
            if heap[0].dist <= heap_reverse[0].dist :
        ...
        if node.closed or node.reverse_closed:
            print("\n\nTERMINATED DOUBLE")
            if reverse:
                path = traverse(nodes,node.id) #+ traverse(nodes,prec_heap_node.id,True)
            else:
                path = traverse(nodes,node.id) #+ traverse(nodes,node.id,True)
            return (path, node.dist ,history,history_reverse)
...
if __name__ == '__main__':
    ...
    Chemin2, Distance2,history2,history2_reverse = dijkstra_double(nodes)
    maze_converter.graphics(Graphic_maze,history2,Chemin2,history_reverse=history2_reverse)
